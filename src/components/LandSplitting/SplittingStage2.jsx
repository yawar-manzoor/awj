...
function SplittingStage2({
    ...
    const [showFillTDDetailsForm, setShowFillTDDetailsForm] = useState(false)
    const [currentSplit, setCurrentSplit] = useState(0)
    const assetIdLastNumber = +assetId.split('-')[3]
    const assetIdInitalString = `${assetId.split('-')[0]}-${
        assetId.split('-')[1]
    }-${assetId.split('-')[2]}-`
    const handleSelectSplit = (e, splitNumber) => {
        setCurrentSplit(splitNumber)
        setNewSplitDetailsList([
            ...newSplitDetailsList,
            {
                ...newSplitDetailsList[splitNumber - 1],
                assetId:
                    assetIdInitalString +
                    (assetIdLastNumber + splitNumber).toString().padStart(5, 0),
            },
        ])
    }
    console.log(newSplitDetailsList)

    return (
        <>
            <div className="flex flex-col gap-6 border-b border-[#EFECE4] pb-5">
                <h3 className="text-[26px] text-[#7A7474]">
                    Splitted Land Details
                </h3>
                <p className="text-[#7B7B7B] font-normal text-base">
                    Choosing a piece of land update further land are and
                ...
                <div className="flex gap-6">
                    <div className=" flex flex-1 gap-4 overflow-x-scroll max-w-[700px] hide-scrollbar">
                        {numberOfSplitsList.map((item, index) => (
                            <div
                                onClick={(e) => handleSelectSplit(e, index + 1)}
                                className={`relative flex-1 flex flex-col gap-5 bg-[#EFECE4] p-8 rounded-xl shrink-0 max-w-[288px] ${
...
export default SplittingStage2
